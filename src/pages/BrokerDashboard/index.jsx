...
export default function BrokerDashboard() {
  ...
  // Backfill class range for schools bookmarked before this data was captured
  // in the stored snapshot — fetch the live record for any school missing it.
  useEffect(() => {
    if (!selected) return;
    ...
    Promise.all(missing.map(s =>
      ...
      setCollections(prev => prev.map(c => c._id === selected._id ? { ...c, schools: patch(c.schools) } : c));
    });
